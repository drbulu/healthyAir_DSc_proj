...
Data set chosen was by state (national is not helpful)

https://www.epa.gov/sites/production/files/2016-12/state_tier1_90-16.xls

Compared to the [Asthma](https://github.com/drbulu/healthyAir_DSc_proj/blob/master/01_Data_Prep/asthma_data_source_prep_02.Rmd) and [Traffic](https://github.com/drbulu/healthyAir_DSc_proj/blob/master/01_Data_Prep/traffic_data_source_prep_01.Rmd) this challenge was arguably simpler.
...
Note: readme sheet of the source data gives an interesting insight into data creation and possible issues. Major note, this data set fused from other data sources, and interpolation used to fill in gaps.
    ...
    * There is extrapolation here (check README sheet of source file). 

However, the granularity (detail level) is comparable to the other datasets that we aim to look at. In addition, a large number of interesting variables are also included. This, therefore, provides a good introduction to the potential value of this dataset while giving us a needed break from the data crunching grind.
