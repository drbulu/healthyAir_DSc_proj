...
processTrafficTable = function(xlsFilePath, monthData, yearData){
    ...
    sheetNames = excel_sheets_quiet(xlsFilePath)
    ...
    if(TRUE %in% checkPageNames){
        ...
        for( sheetID in sheetNameSubset){
            sheetSubset = read_excel_quiet(path = xlsFilePath, sheet = sheetID)
            sheetNum = as.numeric(
...
# https://github.com/tidyverse/readxl/issues/82
excel_sheets_quiet <- function(path) {
...
# get subset of tables above a certain size
# longTables = trafficDataList[unlist(lapply(trafficDataList, FUN = function(x) return(nrow(x) > 153)))]
# get distribution of data table lengths
tableLengths = as.data.frame(table( unlist(lapply(
    trafficDataList, FUN = function(x) return(nrow(x))) )))
...
procTiming = proc.time()
trafficDataList = processFileMetadata(mergedURLTable, isList = T)
proc.time() - procTiming
## Result: ~ 58 sec to ~ 44 sec when replacing 
# sheetNames = names(getSheets(loadWorkbook(xlsFilePath))) (xlsx package)
# with sheetNames = excel_sheets_quiet(xlsFilePath) (readxl package)
# in processTrafficTable() --> also changed excel sheet reader to
# read_excel_quiet() in the same function